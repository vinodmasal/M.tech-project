# -*- coding: utf-8 -*-
"""GRU Model Hyperparameter Tunning.ipynb
...
import pandas as pd
from keras.layers import Dense, GRU, Dropout, GRU 
...
def create_model(learn_rate=0.01):
  ...
  model = Sequential()
  model.add(GRU(units=40, input_shape=(train_X.shape[1], train_X.shape[2])))
...
results.to_csv("GRU_Learning Rate and Momentum.csv")
# files.download('GRU_Learning Rate and Momentum.csv')
...
import pandas as pd
from keras.layers import Dense, GRU, Dropout, GRU, Bidirectional
from sklearn.model_selection import GridSearchCV
from keras.models import Sequential
from keras.layers import Dense
import joblib
...
def create_model(neurons=1):
  ...
  # design GRU network
  model = Sequential()
  model.add(GRU(neurons, input_shape=(train_X.shape[1], train_X.shape[2])))
  model.add(Dropout(0.2))
  model.add(Dense(1))
...
results['params'] =  grid_result.cv_results_['params']
results.to_csv("GRU_Network Weight Initialization.csv")
files.download('GRU_Network Weight Initialization.csv')
...
results['params'] =  grid_result.cv_results_['params']
results.to_csv("GRU_Batch size and epoch.csv")
files.download('GRU_Batch size and epoch.csv')
...
def create_model(init_mode='uniform'):
  ...
  # design GRU network
  model = Sequential()
  model.add(GRU(units=40, kernel_initializer=init_mode, input_shape=(train_X.shape[1], train_X.shape[2])))
...
results['params'] =  grid_result.cv_results_['params']
results.to_csv("GRU_Neuron Activation Function.csv")
files.download('GRU_Neuron Activation Function.csv')
...
results['params'] =  grid_result.cv_results_['params']
results.to_csv("GRU Regularization.csv")
files.download('GRU Regularization.csv')
...
results['params'] =  grid_result.cv_results_['params']
results.to_csv("GRU_Number of Neurons in the Hidden Layer.csv")
files.download('GRU_Number of Neurons in the Hidden Layer.csv')